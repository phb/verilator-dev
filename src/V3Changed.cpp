...
public:
	...
	if (m_chgFuncp == NULL || v3Global.opt.outputSplitCFuncs() < m_numStmts) {
	    ...
	    if (m_tlChgFuncp->stmtsp() == NULL) {
		m_tlChgFuncp->addStmtsp(new AstCReturn(m_scopetopp->fileline(), callp));
	    } else {
		AstCReturn *returnp = m_tlChgFuncp->stmtsp()->castCReturn();
		if (!returnp) m_scopetopp->v3fatalSrc("Internal error on V3Changed.cpp\n");
		// It would be great to be able to use AstLogOr here to allow short curcuiting the change_request
		// functions if work needs to be done. However, V3EmitC has logging statements that prints what variables
		// changed and as such it'd be misleading to short curcuit here.
		AstNode *n = new AstCReturn(m_scopetopp->fileline(), new AstOr(m_scopetopp->fileline(), callp,
...
private:
	...
	// Each change detection function needs at least one AstChangeDet to ensure that V3EmitC outputs the necessary
	// code.
	m_statep->maybeCreateChgFuncp();
