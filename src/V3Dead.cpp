...
//
// Note on packagep
// After the V3Scope/V3LinkDotScoped stage, package links are no longer used, but their presence
// prevents us from removing empty packages. As the links as no longer used after
// V3Scope, we remove them here after scoping to allow more dead node removal.
...
private:
	...
	for (vector<AstCell*>::iterator it = m_cellsp.begin(); it!=m_cellsp.end(); ++it) {
	    AstCell* cellp = *it;
	    if (cellp->user1() == 0 && cellp->modp()->stmtsp() == NULL) {
	...
	for (vector<AstNode*>::iterator it = m_dtypesp.begin(); it != m_dtypesp.end();++it) {
		...
		/*
		 * It's possible that there if a reference to each individual member, but not to the dtype itself.
		 * Check and don't remove the parent dtype if members are still alive.
		 */
...
void V3Dead::deadifyModules(AstNetlist* nodep) {
    ...
    V3Global::dumpCheckGlobalTree("dead_modules.tree", 0, v3Global.opt.dumpTreeLevel(__FILE__) >= 6);
}

void V3Dead::deadifyDTypes(AstNetlist* nodep) {
    UINFO(2,__FUNCTION__<<": "<<endl);
    DeadVisitor visitor (nodep, false, true, false, false);
    V3Global::dumpCheckGlobalTree("dead_dtype.tree", 0, v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}

void V3Dead::deadifyDTypesScoped(AstNetlist* nodep) {
    UINFO(2,__FUNCTION__<<": "<<endl);
    DeadVisitor visitor (nodep, false, true, true, false);
    V3Global::dumpCheckGlobalTree("dead_dtype_scoped.tree", 0, v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}

void V3Dead::deadifyAll(AstNetlist* nodep) {
    UINFO(2,__FUNCTION__<<": "<<endl);
    DeadVisitor visitor (nodep, true, true, false, true);
    V3Global::dumpCheckGlobalTree("dead_all.tree", 0, v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}

void V3Dead::deadifyAllScoped(AstNetlist* nodep) {
    UINFO(2,__FUNCTION__<<": "<<endl);
    DeadVisitor visitor (nodep, true, true, true, true);
    V3Global::dumpCheckGlobalTree("dead_all_scoped.tree", 0, v3Global.opt.dumpTreeLevel(__FILE__) >= 3);
}
