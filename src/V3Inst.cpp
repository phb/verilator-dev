...
private:
	...
	if (nodep->rangep()) {
	    ...
	    bool isIface = varIface
		&& varIface->dtypep()->castUnpackArrayDType()
		&& varIface->dtypep()->castUnpackArrayDType()->subDTypep()->castIfaceRefDType ();
	    ...
	    m_cellRangep=NULL;
	    if (isIface) {
		varIface->unlinkFrBack(); pushDeletep(varIface); VL_DANGLING(varIface);
	    }
